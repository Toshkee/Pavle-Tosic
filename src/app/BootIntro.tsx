...
export default function BootIntro() {
  ...
  const skipRef = useRef<(() => void) | null>(null); // lets the skip button call startLeave
  ...
  useEffect(() => {
    ...
    root.style.overflow = "hidden"; // lock scroll while booting

    const timers: ReturnType<typeof setTimeout>[] = [];
    let i = 0;

    // Function declarations (hoisted) so startLeave/onKey can reference each
    // other regardless of order.
    function startLeave() {
      if (leaveTimer.current) return; // idempotent: auto-advance + skip may both fire
      // Detach the global key listener NOW. The component returns null after
      // "done" but never unmounts, so the cleanup below would otherwise never
      // run — leaving Enter/Esc/Space preventDefault'd for the whole session
      // (which would block typing spaces into the Terminal and spacebar scroll).
    ...
    function tick() {
      ...
      if (i < STEPS.length) {
        ...
        // Boot log done — hold on the "press Enter" prompt and auto-enter
        // after 5s if the visitor doesn't press anything.
        setReady(true);
        timers.push(setTimeout(startLeave, 5000));
      }
    }

    timers.push(setTimeout(tick, 280));
    window.addEventListener("keydown", onKey);
    skipRef.current = startLeave; // let the skip button call it
  ...
  useEffect(() => {
    if (phase !== "boot") document.documentElement.style.overflow = "";
  }, [phase]);
  ...
  return (
    ...
    >
      <div className="w-full max-w-md font-mono text-[13px] leading-relaxed">
        ...
        {ready ? (
          <div className="mt-4">
            {/* The prompt itself is the click target, so the keyboard and mouse
                paths share one line of copy (no redundant second "enter"). */}
            <button
              type="button"
            ...
            {/* Drains over 5s; when it empties the site auto-opens. */}
