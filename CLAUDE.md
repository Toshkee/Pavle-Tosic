...
| `contact` | `Order.tsx` | Closing scene, no card: poster-size heading over the night-train slide, email CTA, text links; from md up the type ends above ~62% of the viewport so the train crosses the bridge under it; the footer continues its scrim. |
| | `Footer.tsx`, `Nav.tsx` | Floating glass nav with an IntersectionObserver active pill and a phone menu. |
| | `MorphBackdrop.tsx` | Fixed full-viewport WebGL backdrop: four AI-generated anime paintings in the hero's style (`public/images/morph/`: dusk countryside for About, sky islands for Work/Client work, a moonlit sky for Stack (a sunset painting graded to night offline), night train for Log/Contact; each slide owns a run of `sectionIds`), a noise burn-through morph when the owning slide changes, a still between morphs, a slide `plane` (the night train's cars cut out as `night-train-cars.webp` over a re-plated bridge, crossing it in a CSS loop, DOM-aligned to the shader's cover framing), plus a per-section weather layer (`Ambience.tsx`: fireflies, sunset motes, twinkling stars, water mist; soft round points, CSS only, paused under the hero). `<img>` cross-fade fallback mounts only if WebGL fails. Keyed by the section ids above. |
